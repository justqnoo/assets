(function () {
    console.clear();
    console.log("%cbyzan.lol client injected - v0.2", "color:#8a7bff;font-weight:bold;font-size:20px;");
    ...
    const DEFAULT_SETTINGS = {
        orbAutoClick: true, atomAutoClick: true, CPS: 500, orbClickDelay: 0,        // ← 500 CPS by default now
        notificationsEnabled: true, outlineColor: "#6a5acd", glowColor: "#8a7bff",
        notifColor: "#8a7bff", headerLineEnabled: true
    };
    ...
    document.head.insertAdjacentHTML("beforeend", `
        <style>
            ...
            .win-fade { opacity: 0; animation-fill-mode: forwards; }
            .win-fade.fade-in { animation: fadeIn 0.28s cubic-bezier(.67,0,.33,1) forwards; }
            .win-fade.fade-out { animation: fadeOut 0.28s cubic-bezier(.67,0,.33,1) forwards; }
    ...
    const notify = text => {
        ...
        Object.assign(n.style, {
            ...
            boxShadow:"0 8px 30px rgba(0,0,0,0.5)", animation:"popIn 0.4s ease"
        });
    ...
    const headerWin = createWin("byzan.lol client v0.2", "", false, "280px");
    ...
    const miscWin = createWin("Miscellaneous", `
        ...
        <div id="atomContent" style="display:none;padding-top:8px">
            ...
            <span style="color:#8a7bff;font-size:12px"> (max 10,000!)</span>
        </div>
    `, true, "280px");
    ...
    miscWin.body.querySelector("#cpsInput").onchange = e => {
    ...
    visualWin.body.querySelector("#hline").onchange = function() {
        headerLineEnabled = this.checked;
    ...
    document.addEventListener("keydown", e => {
        if (e.key === "Tab" && !e.ctrlKey && !e.altKey) {
            e.preventDefault();
            visible = !visible;
            const windows = [headerWin.win, miscWin.win, visualWin.win, notifWin.win];
            windows.forEach(win => {
                win.classList.remove("fade-in", "fade-out");
                if (visible) { win.style.display = "block"; void win.offsetWidth; win.classList.add("fade-in"); }
                else { win.classList.add("fade-out"); setTimeout(() => { if(!visible) win.style.display="none"; win.classList.remove("fade-out"); }, 300); }
            });
        }
    });
    ...
    overlay.innerHTML = `<div class="smooth" style="background:#111;border:4px solid #ff4444;border-radius:18px;padding:45px 40px;max-width:500px;color:white;text-align:center;font-family:'Poppins',sans-serif;box-shadow:0 0 60px #f4444480;">
        <h2 style="color:#ff4444;margin:0 0 20px;font-size:28px;">Warning!</h2>
        <p style="font-size:19px;margin:20px 0;"><strong>I am not responsable for your actions.</strong> Enjoy byzan.client</p>
        <p style="font-size:18px;margin:20px 0;">Automation may violate ToS — <strong>use at your own risk</strong>.</p>
        <button id="ok" style="background:#ff4444;padding:16px 50px;border:none;border-radius:12px;color:white;font-weight:700;cursor:pointer;font-size:17px;">Okay!</button>
    </div>`;
    ...
    overlay.querySelector("#ok").onclick = () => {
        ...
        [headerWin.win, miscWin.win, visualWin.win, notifWin.win].forEach(el => { el.style.display="block"; el.classList.add("fade-in"); });
        applyHeaderLine(headerLineEnabled);
        position();
    };

    setTimeout(() => { applyHeaderLine(headerLineEnabled); position(); }, 100);
})();
