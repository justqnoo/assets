(function() {
  ...
  console.log("david / justqnoo@github mrdavidss@discord") const font = document.createElement("link");
  font.rel = "stylesheet";
  ...
  notifStyles.innerHTML = `@keyframes growIn {
    0 % {
      opacity: 0;
    ...
    animation: growIn.4s ease forwards;
  }.popIn {
  ...
  const miscWin = createWindow("Miscellaneous", 250, ` < label > <input type = "checkbox"id = "orbToggle" > Auto Orb Clicker < /label><br><br>
<label><input type="checkbox" id="atomToggle"> Auto Atom Clicker</label > <br > <br > <div id = "cpsDropdown"style = "background:#222;padding:6px;border-radius:6px;cursor:pointer;margin-bottom:8px;text-align:center;" > Auto Atom Clicker Options▼ < /div>
<div id="cpsContent" style="display:none;padding-left:8px;">
<label>CPS: <input id="cpsInput" type="number" style="width:60px;background:#111;color:white;border:1px solid #555;border-radius:4px;padding:3px;"></label > </div>
`);
const notifWin = createWindow("Notifications", 250, `<label><input type="checkbox" id="notifToggle"> Enable Notifications</label > `);
  ...
  settingsButton.addEventListener("mouseenter", () = >{
    settingsButton.style.boxShadow = `0 0 20px $ {
      glowColor
    },
    0 0 40px $ {
      glowColor
  ...
  settingsButton.addEventListener("mouseleave", () = >{
    settingsButton.style.boxShadow = `0 0 10px $ {
      glowColor
    }`;
  ...
  const settingsWin = createWindow("Visual Settings", 250, ` < label > Outline Color: <input type = "color"id = "outlineColorInput"value = "${outlineColor}" > </label><br><br>
<label>Glow Color: <input type="color" id="glowColorInput" value="${glowColor}"></label > <br > <br > <label > Notification Color: <input type = "color"id = "notifColorInput"value = "${notifColor}" > </label><br><br>
<label><input type="checkbox" id="headerLineToggle"> Enable Header Line</label > `);
  ...
  function updateAllStyles() { [headerWin, miscWin, notifWin, settingsWin].forEach(w = >{
      w.win.style.border = `2px solid $ {
        outlineColor
      }`;
      w.win.style.boxShadow = `0 0 10px $ {
        glowColor
      }`;
  ...
  function updateAllHeaders() { [headerWin, miscWin, notifWin, settingsWin].forEach(w = >updateHeaderLine(w.header));
  }
  ...
  atomToggle.addEventListener("change", e = >{
    ...
    for (const mut of mutations) {
      mut.addedNodes.forEach(node = >{
        if (node.nodeType === 1 && node.classList.contains("power-up") && node.classList.contains("bonus-atom")) {
          yellowOrbCount++;
          node.click();
          console.log(` % cYellow Orb Logged($ {
            yellowOrbCount
          })–$ {
            new Date().toLocaleTimeString()
          }`, "color:yellow;font-weight:bold;");
          sendNotification(`byzan client: Yellow Orb Count: $ {
            yellowOrbCount
          }`);
        }
      });
